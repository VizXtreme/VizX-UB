...
if command -v termux-setup-storage; then
  printf "${RED}For termux, please use https://raw.githubusercontent.com/The-MoonTg-project/VizX-UB/main/termux-install.sh${NC}\n" # skipcq
  exit 1
fi
...
if [[ -d "VizX-UB" && "$(basename "$PWD")" != "VizX-UB" ]]; then
  cd VizX-UB || exit 2
elif [[ "$(basename "$PWD")" == "VizX-UB" && -f ".env.dist" && -f "main.py" && -d "modules" ]]; then
  printf "${BLUE}Already inside the VizX-UB repo, proceeding...${NC}\n" # skipcq
else
  git clone https://github.com/The-MoonTg-project/VizX-UB || exit 2
  cd VizX-UB || exit 2
fi

if [[ -f ".env" ]] && [[ -f "my_account.session" ]]; then
  printf "${GREEN}It seems that VizX-UB is already installed. Exiting...${NC}\n" # skipcq
  exit
fi
...
# Adjust the ownership of the VizX-UB directory
...
while true; do
  ...
  1)
    ...
    su -c "pm2 start main.py --name VizX --interpreter python3" $SUDO_USER
    su -c "pm2 save" $SUDO_USER

    printf "${GREEN}============================\\n" # skipcq
    printf "Great! VizX-UB installed successfully and running now!\n"
    printf "Installation type: PM2\n"
    printf "Start with: \"pm2 start VizX\"\n"
    printf "Stop with: \"pm2 stop VizX\"\n"
    printf "Process name: VizX\n"
    printf "============================${NC}\n" # skipcq
    break
    ;;
  2)
    cat >/etc/systemd/system/VizX.service <<EOL
[Unit]
Description=Service for VizX-UB
[Service]
Type=simple
...
EOL
    systemctl daemon-reload
    systemctl start VizX
    systemctl enable VizX

    printf "${GREEN}============================\\n" # skipcq
    printf "Great! VizX-UB installed successfully and running now!\n"
    printf "Installation type: Systemd service\n"
    printf "Start with: \"sudo systemctl start VizX\"\n"
    printf "Stop with: \"sudo systemctl stop VizX\"\n"
    printf "============================${NC}\n" # skipcq
    break
    ;;
  3)
    printf "${GREEN}============================\\n" # skipcq
    printf "Great! VizX-UB installed successfully!\n"
...
# Adjust the ownership of the VizX-UB directory again as a final step
chown -R $SUDO_USER:$SUDO_USER .
